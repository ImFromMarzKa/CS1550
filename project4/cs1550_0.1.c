...
/*Function to finds a requested directory
	...
	* Returns:
		...
		*else()
			...
			* entry = null
*/
static int get_directory(char *directory, struct cs1550_directory_entry *dir_entry){
	FILE *file;
...
static int cs1550_getattr(const char *path, struct stat *stbuf)
{
	...
	if (strcmp(path, "/") == 0) {
		...
		if(num == 1){
			//Search array of directories
			int i;
			int found = 0;
			for(i = 0; i < cs1550_root_directory.nDirectories; i++){
				if(!strcmp(cs1550_root_directory.directories[i].dname, directory)){
					found = 1;
					break;
				}
			}
		...
		//Check if name is a regular file
			...
			int i;
			int found = 0;
			//Search for subdirectory from path
			for(i = 0; i < cs1550_root_directory.nDirectories; i++){
				if(!strcmp(cs1550_root_directory.directories[i].dname, directory)){
					int j;
					//Search for filename from path within subdirectory from path
					for(j = 0; j < cs1550_root_directory.directories[i].nFiles; j++){
						if(!strcmp(cs1550_root_directory.directories[i].files[j].fname, filename)){
							found = 1;
							break;
						}
					}

		...
		for(i = 0; i < root_dir->nDirectories; i++){
			...
			if(!strcmp(root_dir->directories[i].dname, directory)){
				...
				if(fseek(file, off, SEEK_SET)){
					...
					entry = (struct cs1550_directory_entry *)file;
				}
				break;
			}
		}
...
static int cs1550_mknod(const char *path, mode_t mode, dev_t dev)
{
	...
	dir_entry->files[nFiles].nStartBlock = free_block;
	dir_entry->files[nFiles].fsize = 0;
	...
	if(!fwrite(dir_entry->files[nFiles], sizeof(struct cs1550_file_directory), 1, file)){
		fprintf(stderr, "Failed to write to .disk\n");
		fclose(file);
		return -1;
	}
...
//IF THERE IS AN ERROR, IT IS PROBABLY HERE
	int start_loc = file_loc;
	//Check if file is larger than one block
	if(file_size > BLOCK_SIZE){
		...
		//Calculate the total number of blocks spanned by the file
		// tot_blocks = ceiling(file_size / BLOCK_SIZE);
		int tot_blocks = file_size / BLOCK_SIZE;
		if((file_size % BLOCK_SIZE) > 0){
			num_blocks++;
		}

		//Check which block to begin
		// & update num_blocks_2_read
		int num_blocks_2_read = tot_blocks;
		while(offset > BLOCK_SIZE){
			offset -= BLOCK_SIZE;
			num_blocks_2_read--;
			start_loc = fat->table[start_loc];
		}
